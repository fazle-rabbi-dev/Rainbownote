...
export const Dashboard = () => {
  ...
  const handleSearch = (e) => {
    ...
    const filteredNotes = notes.filter(note => note.title.toLowerCase().startsWith(value.toLowerCase()))
  ...
  return (
    <div className="px-4 py-2">
      <div className="relative flex gap-1">
        <input onChange={handleSearch} placeholder="Search notes" className="dashboard_search" type="text" name="" id="" value={searchValue} />
        <span className="absolute top-2 left-2 text-gray-400"><Search /></span>
      </div>
      ...
      
      {
        notes?.length === 0 && (
            <div className="">
              <span className="no_result_found">You haven't created any notes yet.</span>
            </div>
          )
      }
